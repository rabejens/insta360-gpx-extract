...
for n in win-x86 win-x64 linux-x64 osx-x64 osx-arm64; do
	...
	if [ -f Insta360GPXExtract/bin/Release/net6.0/$n/publish/Insta360GPXExtract.exe ]; then
		cp -f Insta360GPXExtract/bin/Release/net6.0/$n/publish/Insta360GPXExtract.exe build/$n
	else
		cp -f Insta360GPXExtract/bin/Release/net6.0/$n/publish/Insta360GPXExtract build/$n
	fi
done
